...
import stanza.rl_tools.simple_policy as simple_policy
import stanza.rl_tools.critic_nets_mlp as critic_nets_mlp
from stanza.rl_tools.flax_models import Batch, InfoDict, Model, PRNGKey
from stanza.rl_tools.iql_update import update_q, update_v, update_a
...
def _update_jit(
    ...
    new_actor, actor_info = update_a(key, actor, target_critic,
...
class Learner(object):
    def __init__(self,
        ...
        actor_def = simple_policy.NormalTanhPolicy(hidden_dims,
                                            action_dim,
                                            log_std_scale=1e-3,
                                            log_std_min=-5.0,
        ...
        critic_def = critic_nets_mlp.DoubleCritic(hidden_dims)
        critic = Model.create(critic_def,
                              inputs=[critic_key, observations, actions],
                              tx=optax.adam(learning_rate=critic_lr))

        value_def = critic_nets_mlp.ValueCritic(hidden_dims)
    ...
    def sample_actions(self,
        ...
        rng, actions = simple_policy.sample_actions(self.rng, self.actor.apply_fn,
