...
def quadratize(fun, argnums=3):
  ...
  """
  def quadratizer(xs, us, *args):
    # Does a bunch of nonsense to
    # extract the right jacobians
    def f_fun(x, u, t, xs, us, *args):
      xs = xs.at[t].set(x)
      us = us.at[t].set(u)
      return fun(xs, us, *args)
    ...
    return hessian_x(xs, us, t, xs, us, *args), \
        ...
        hessian_x_u(xs, us,t, xs, us, *args)
...
def grad_wrt_controls(cost, dynamics, U, x0, cost_args, dynamics_args):
  ...
  A, B = jacobians(X, pad(U), *dynamics_args)
  q, r = grad_cost(X, pad(U), *cost_args)
